<template>
	<v-dialog
		v-model="toggleClassroomScheduleUpdateModal"
		...
		<v-card class="pa-5">
			...
			<v-card-actions class="pa-0 mt-4">
				...
				<v-btn
					...
					@click="toggleClassroomScheduleUpdateModal = false"
				>
					Cancel
				</v-btn>
...
<script>
	...
	export default {
		name: 'ClassroomScheduleUpdateModal',
		props: {
			classSchedule: {
				required: true
			},
			toggleClassroomScheduleUpdateModal: {
				type: Boolean,
				required: true
			}
		},
		...
		data () {
			return {
				...
				defaultValueCourseID: null,
				defaultValueCourseTitle: null,
				defaultValueCourseCode: null,
				defaultValueCourseSection: null,
				defaultValueCourseTimeStart1: null,
				defaultValueCourseTimeStart2: null,
				defaultValueCourseTimeEnd1: null,
				defaultValueCourseTimeEnd2: null,
				defaultValueCourseRoom1: null,
				defaultValueCourseRoomNum1: null,
				defaultValueCourseRoom2: null,
				defaultValueCourseRoomNum2: null,
		...
		watch: {  
			classSchedule () {
				this.courseTitle = this.classSchedule.title
				this.courseCode = this.classSchedule.code
				this.courseSection = this.classSchedule.section
				this.courseUnits = this.classSchedule.units
				this.courseType = this.classSchedule.type
				this.courseTimeStart1 = this.classSchedule.timeStart1
				this.courseTimeEnd1 = this.classSchedule.timeEnd1
				this.courseDay1 = this.classSchedule.day1
				this.courseRoom1 = this.classSchedule.room1
				this.courseRoomNum1 = this.classSchedule.roomNum1

				if (this.classSchedule.day2 === 'None') {
					this.courseDuration = 'single'
				} else {
					this.courseDuration = 'split'
					this.courseDay2 = this.classSchedule.day2
				}

				if (this.classSchedule.timeStart2 === '00:00:00') {
					this.courseTimeStart2 = null
				} else {
					this.courseTimeStart2 = this.classSchedule.timeStart2
				}

				if (this.classSchedule.timeEnd2 === '00:00:00') {
					this.courseTimeEnd2 = null
				} else {
					this.courseTimeEnd2 = this.classSchedule.timeEnd2
				}

				if (this.classSchedule.room2 === 'None') {
					this.courseRoom2 = null
				} else {
					this.courseRoom2 = this.classSchedule.room2
				}

				if (this.classSchedule.roomNum2 === 'None') {
					this.courseRoomNum2 = null
				} else {
					this.courseRoomNum2 = this.classSchedule.roomNum2
				}

				this.defaultValueCourseID = this.classSchedule.id
				this.defaultValueCourseTitle = this.classSchedule.title
				this.defaultValueCourseCode = this.classSchedule.code
				this.defaultValueCourseSection = this.classSchedule.section
				this.defaultValueCourseTimeStart1 = this.classSchedule.timeStart1
				this.defaultValueCourseTimeStart2 = this.classSchedule.timeStart2
				this.defaultValueCourseTimeEnd1 = this.classSchedule.timeEnd1
				this.defaultValueCourseTimeEnd2 = this.classSchedule.timeEnd2
				this.defaultValueCourseRoom1 = this.classSchedule.room1
				this.defaultValueCourseRoomNum1 = this.classSchedule.roomNum1
				this.defaultValueCourseRoom2 = this.classSchedule.room2
				this.defaultValueCourseRoomNum2 = this.classSchedule.roomNum2
			},
			courseTitle () {
				if (this.toggleClassroomScheduleUpdateModal === true) {
			...
			toggleClassroomScheduleUpdateModal () {
				...
				if (this.toggleClassroomScheduleUpdateModal === false) {
					this.$emit('close-classroom-schedule-update-modal')
				}
			}
		},
		computed: {
		...
		methods: {
			...
			submit () {
				...
				if (this.courseCode.length == 7 && this.courseType === 'Lab') {
					this.formErrors.push('Invalid course code for laboratory class!')
				}

				if (this.courseCode.length == 8 && this.courseType === 'Lec') {
				...
				if (this.courseDuration === 'single' && this.courseDay1 !== null && this.courseTimeStart1 !== null && this.courseTimeEnd1 !== null && this.courseRoom1 !== null && this.courseRoomNum1 !== null) {
					...
					for (let w = 0; w < tempSchedule.length; w++) {
						if (this.courseTitle !== this.defaultValueCourseTitle || this.courseCode !== this.defaultValueCourseCode || this.courseSection !== this.defaultValueCourseSection) {
							if (this.courseTitle === tempSchedule[w].name && this.courseCode === tempSchedule[w].code && this.courseSection === tempSchedule[w].section && this.courseUnits === tempSchedule[w].units) {
								this.formErrors.push(`${tempSchedule[w].name} - ${tempSchedule[w].code} (${tempSchedule[w].section}) already exist!`)
					...
					if (isScheduleExist === false) {
						const checkSchedule = tempSchedule.filter(data => data.day == this.courseDay1 && data.id != this.defaultValueCourseID)

						if (this.courseTimeStart1 != this.defaultValueCourseTimeStart1 || this.courseTimeEnd1 != this.defaultValueCourseTimeEnd1 || this.courseRoom1 != this.defaultValueCourseRoom1 || this.courseRoomNum1 != this.defaultValueCourseRoomNum1) {
				...
				// check time overlap on split day schedule
				if (this.courseDuration === 'split' && this.courseDay1 !== null && this.courseDay2 !== null && this.courseTimeStart1 !== null && this.courseTimeEnd1 !== null && this.courseTimeStart2 !== null && this.courseTimeEnd2 !== null && this.courseRoom1 !== null && this.courseRoomNum1 !== null && this.courseRoom2 !== null && this.courseRoomNum2 !== null) {
					...
					if (isScheduleExist2 === false) {
						const checkSchedule = tempSchedule.filter(data => data.day == this.courseDay1 && data.id != this.defaultValueCourseID)
						const checkSchedule2 = tempSchedule.filter(data => data.day == this.courseDay2 && data.id != this.defaultValueCourseID)
						...
						// Day 2 Schedule
						if (this.courseTimeStart2 != this.defaultValueCourseTimeStart2 || this.courseTimeEnd2 != this.defaultValueCourseTimeEnd2 || this.courseRoom2 != this.defaultValueCourseRoom2 || this.courseRoomNum2 != this.defaultValueCourseRoomNum2) {
							for (let w = 0; w < checkSchedule2.length; w++) {
								if ((this.courseTimeStart2 < checkSchedule2[w].end) && (checkSchedule2[w].start < this.courseTimeEnd2)) {
									if (this.courseRoom2 === checkSchedule2[w].room && this.courseRoomNum2 === checkSchedule2[w].roomNum) {
										...
										this.formErrors.push(`Day 2 schedule is conflict on ${checkSchedule2[w].name} (${checkSchedule2[w].section}) - ${checkSchedule2[w].day} ${checkSchedule2[w].start} - ${checkSchedule2[w].end} on ${checkSchedule2[w].room}${checkSchedule2[w].roomNum}`)
									}
								}
							}
						}
				...
				if (!this.$v.$invalid && this.formErrors.length === 0) {
					if (this.courseDuration === 'single') {
						axios.patch(this.axiosBaseURL + '/schedule/' + this.classSchedule.id, {
							...
							prof_id: this.classSchedule.prof
						})
						.then(response => {
							if (this.classSchedule.prof !== 'N/A') {
								const facultySchedule = this.mainFacultyScheduleList.filter(data => data.classID == this.classSchedule.id)

								axios.patch(this.axiosBaseURL + '/faculty/' + facultySchedule[0].id, {
									name: this.courseTitle,
									class_id: facultySchedule[0].classID,
									c_code: this.courseCode,
									s_code: this.courseSection,
									units: this.courseUnits,
									day: this.courseDay1,
									...
									room_num_2: 'None',
									type: this.courseType,
									prof_id: facultySchedule[0].prof,
									sub_id: facultySchedule[0].subProf
								})
								.catch(error => {
									this.$loading(false)
							...
							this.$emit('close-classroom-schedule-update-modal', false)
							this.$loading(false)
							this.$emit('show-classroom-schedule-update-notification', response.data)
						})
						.catch(error => {
							this.$loading(false)
							this.formErrors.push(error)
