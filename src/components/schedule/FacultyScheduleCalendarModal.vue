<template>
	<v-dialog
		v-model="toggleFacultyScheduleCalendarModal"
	...
	>
		...
		>
			<v-card-title class="font-weight-medium pa-0">
				...
				<v-btn
					...
					@click="toggleFacultyScheduleCalendarModal = false"
				>
					...
					<v-col>
						<v-sheet height="505">
							<v-calendar
								...
								:events="facultyScheduleEvents"
								:event-overlap-mode="defaultCalendarMode"
...
<script>
	...
	export default {
	name: 'FacultyScheduleCalendarModal',
	props: {
		schedulePayload: {
			required: true
		},
		toggleFacultyScheduleCalendarModal: {
			type: Boolean,
			required: true
		}
	},
	...
	watch: {
		toggleFacultyScheduleCalendarModal() {
			if (this.toggleFacultyScheduleCalendarModal === false) {
				this.$emit('close-faculty-schedule-calendar-modal')
			}
		}
	},
	computed: {
	...
	methods: {
		setEvents() {
			...
			const tempSchedule = this.schedulePayload
