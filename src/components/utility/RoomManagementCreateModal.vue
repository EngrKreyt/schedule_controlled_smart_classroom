	...
	>
		<v-card class="pa-5">
			...
			<v-card-text class="pa-0">
				...
				>
					<v-row :class="roomBuilding === null ? 'mt-4' : 'mt-8'">
						<v-col
							cols="12"
							sm="6"
							class="pt-0"
			...
			<v-card-actions class="pa-0 mt-4">
				...
				<v-btn
					...
					color="transparent"
					@click="toggleRoomCreateModal = false"
				>
					Cancel
				</v-btn>
...
<script>
	...
	export default {
		name: 'RoomManagementCreateModal',
		props: {
		...
		watch: {
			roomBuilding () {
				if (this.toggleRoomCreateModal === true) {
			...
			toggleRoomCreateModal () {
				...
				if (this.toggleRoomCreateModal === false) {
					this.$emit('close-room-create-modal')
				}
			}
		},
		computed: {
		...
		methods: {
			...
			submit () {
				...
				const roomList = this.mainRoomList

				if (this.roomBuilding !== null && this.roomNumber !== null && this.roomType !== null) {
					for (let w = 0; w < roomList.length; w++) {
						if (this.roomBuilding === roomList[w].building && this.roomNumber === roomList[w].number) {
							this.roomBuildingValidation = true
							this.roomNumberValidation = true
							this.formErrors.push('Room ' + roomList[w].building + roomList[w].number + ' already exist!')
						}
					}
				}
				...
				if (!this.$v.$invalid && this.formErrors.length === 0) {
					axios.post(this.axiosBaseURL + '/room', {
					...
					.then(response => {
						...
						this.$emit('close-room-create-modal')
						this.$loading(false)
						this.$emit('show-room-create-notification', response.data)
					})
					.catch(error => {
						this.$loading(false)
