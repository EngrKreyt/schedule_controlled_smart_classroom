<template>
	<v-dialog
		v-model="toggleUserAccountCalendarModal"
		...
		>
			<v-card-title class="font-weight-regular primary--text pa-0">
				...
				<v-btn
					...
					small
					depressed
					class="font-weight-regular body-2 primary--text"
					color="transparent"
					@click="toggleUserAccountCalendarModal = false"
				>
...
<script>
	...
	export default {
	name: 'UserAccountCalendarModal',
	props: {
		schedulePayload: {
			required: true
		},
		toggleUserAccountCalendarModal: {
			type: Boolean,
			required: true
		}
	},
	...
	watch: {
		toggleUserAccountCalendarModal() {
			if (this.toggleUserAccountCalendarModal === false) {
				this.$emit('close-user-account-calendar-modal')
			}
		}
	},
	computed: {
